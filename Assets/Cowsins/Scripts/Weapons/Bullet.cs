...
public class Bullet : MonoBehaviour
{
    ...
    private void OnTriggerEnter(Collider other)
    {
        ...
        if (!explosionOnHit)
        {
            ...
            {
                other.transform.parent.GetComponent<IDamageable>().Damage(damage * criticalMultiplier, contact);
                DestroyProjectile();
                hit = true;
                return;
            }
            ...
            {
                other.GetComponent<IDamageable>().Damage(damage, contact);
                DestroyProjectile();
                hit = true;
                return;
            }        
            ...
            foreach (Collider c in cols)
            {
                if (c.GetComponent<IDamageable>() != null)
                {
                    if (c.CompareTag("Player") && hurtsPlayer)
                    {
                        ...
                        c.GetComponent<IDamageable>().Damage(dmg, transform.position);
                    }
