...
public class WeaponController : MonoBehaviour
{
    ...
    public void MeleeAttack(float attackRange, float damage)
    {
        ...
        foreach (var c in col)
        {
            ...
            {
                if (c.CompareTag("Critical")) c.transform.parent.GetComponent<IDamageable>().Damage(dmg, Vector3.zero);
                else c.GetComponent<IDamageable>().Damage(dmg, Vector3.zero);
            }
    ...
    private void Hit(LayerMask layer, float damage, RaycastHit h, bool damageTarget)
    {
        ...
        if (h.collider.gameObject.CompareTag("Critical")) h.collider.transform.parent.GetComponent<IDamageable>().Damage(damage * weapon.criticalDamageMultiplier * GetDistanceDamageReduction(h.collider.transform), h.point);
        else if (h.collider.GetComponent<IDamageable>() != null) h.collider.GetComponent<IDamageable>().Damage(damage * GetDistanceDamageReduction(h.collider.transform), h.point);
    }
